...
export default function ProviderUsageView({ provider }: { provider: "gpt" | "gemini" }) {
  const { data: models, mutate: mutateModels } = useLlmModels(provider);
  const { data: cost, mutate: mutateCost } = useLlmCost(provider);
  const { data: rateLimits, mutate: mutateRateLimits } = useLlmRateLimits(provider);
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-xs font-semibold text-sm-text-dim uppercase tracking-wide">GPT Details</h2>
        <button
        ...
        {rateLimits?.available && (rateLimits.windows?.length ?? 0) > 0 ? (
          ...
          <div className="text-xs text-sm-text-dim">
            {rateLimits?.available
              ? `현재 활성 ${title} 세션이 없습니다.`
              : "rate-limit 데이터를 불러오지 못했습니다. 새로고침해주세요."}
          </div>
        )}
      </div>
